<?php
$getusers = "SELECT DISTINCT u.iduser, u.idcard, u.nom, u.prenom, u.email, u.telephone
             FROM users u
             INNER JOIN message m ON u.iduser = m.idfrom
             WHERE m.idto = $idadmin
             AND m.type = 'u'
             AND m.statut = 0";
...
if ($users->num_rows > 0) {
    while ($row = $users->fetch_assoc()) {
        ...
        $nbrnewmsgsSQL = "SELECT COUNT(*) as total FROM message 
                          WHERE idfrom = ".$row['iduser']." 
                          AND idto = $idadmin 
                          AND type = 'u' 
                          AND statut = 0";
        ...
        if ($nbrmsgsresult && $rowmsgs = $nbrmsgsresult->fetch_assoc()) {
            $nbrmsgs = $rowmsgs['total'];
        }
?>
<li>
    <a href="reclamations.php?idcon=<?php echo $row['iduser']; ?>">
        ...
        <p>
            ...
            <?php if ($nbrmsgs > 0) echo $nbrmsgs . " Nouveaux message"; ?>
        </p>
    </a>
</li>
<?php
				...
				<div class="user-info-dropdown">
					<div class="dropdown">
						...
						>
							...
							<?php
								echo (isset($prenom) && isset($nom)) ? $prenom . ' ' . $nom : 'Admin';
							?></span>
