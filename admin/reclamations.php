...
// Send message
if (isset($_POST['send']) && isset($_GET['idcon'])) {
    $idto = (int)$_GET['idcon'];
    $idfrom = (int)$idadmin;
    $msg = mysqli_real_escape_string($DB->conn, $_POST['msg']);

    $sendmsgSQL = "INSERT INTO message(idfrom, idto, message, type) VALUES($idfrom, $idto, '$msg', 'a')";
    $DB->executeSQL($sendmsgSQL);
}

// Fetch messages
if (isset($_GET['idcon'])) {
    $idcon = (int)$_GET['idcon'];

    $fetchMessagesSQL = "SELECT * FROM message WHERE 
                        (idfrom = $idadmin AND idto = $idcon) 
                        OR 
                        (idfrom = $idcon AND idto = $idadmin) 
                        ORDER BY idmessage ASC";
    $result = $DB->executeSQL($fetchMessagesSQL);

    if (mysqli_num_rows($result) > 0) {
        while ($row = mysqli_fetch_assoc($result)) {
            $sender = ($row['idfrom'] == $idadmin) ? "You" : "Client";
            echo "<p><strong>{$sender}:</strong> " . htmlspecialchars($row['message']) . "</p>";
        }
    } else {
